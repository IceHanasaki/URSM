...
from ursm.losses import DisparityLoss, CalibrationAwareSmoothness, MultiScaleLoss
from ursm.utils import visualize_disparity, compute_epe, compute_bad_pixels


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='Train URSM model')
...
def create_model(config, device):
    ...
    model = URSMNet(
        ...
        refine_iterations=config['model']['refine_iterations'],
        estimate_calibration=config['model']['estimate_calibration']
    )
...
def train_epoch(model, train_loader, optimizer, disparity_loss_fn, smoothness_loss_fn, 
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        # Total loss
        loss = disparity_loss + smoothness_loss
